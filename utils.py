...
def checkpoint(config, epoch, gen, dis):
    ...
    net_gen_model_out_path = os.path.join(
        model_dir, 'gen_model_epoch_{}.pth'.format(epoch))
    net_dis_model_out_path = os.path.join(
        model_dir, 'dis_model_epoch_{}.pth'.format(epoch))
...
def carl_error(y_true, csm, y_pred):
    ...
    clearmask = torch.ones_like(csm) - csm
    predicted = y_pred
    input_cloudy = y_pred
    target = y_true

    cscmae = torch.mean(clearmask * torch.abs(predicted - input_cloudy) + csm * torch.abs(
        predicted - target)) + 1.0 * torch.mean(torch.abs(predicted - target))
