...
def train(config):
    ...
    # initialize datasets(train & validation)
    ...
    training_data_loader = DataLoader(
        dataset=train_dataset, num_workers=config.threads, batch_size=config.batchsize, shuffle=True)
    validation_data_loader = DataLoader(
        dataset=validation_dataset, num_workers=config.threads, batch_size=config.validation_batchsize, shuffle=False)
    ...
    dis = Discriminator(in_ch=config.in_ch,
                        out_ch=config.out_ch, gpu_ids=config.gpu_ids)
    ...
    # setup optimizer
    opt_gen = optim.Adam(gen.parameters(), lr=config.lr, betas=(
        config.beta1, 0.999), weight_decay=0.00001)
    opt_dis = optim.Adam(dis.parameters(), lr=config.lr, betas=(
        config.beta1, 0.999), weight_decay=0.00001)

    real_a = torch.FloatTensor(
        config.batchsize, config.in_ch, config.width, config.height)
    real_b = torch.FloatTensor(
        config.batchsize, config.out_ch, config.width, config.height)
    M = torch.FloatTensor(config.batchsize, config.width, config.height)
    shadow_mask = torch.FloatTensor(
        config.batchsize, config.out_ch, config.width, config.height)
    ...
    for epoch in range(1, config.epoch + 1):
        ...
        for iteration, batch in enumerate(training_data_loader, 1):
            # get cloud img,ground truth and M from batch
            real_a_cpu, real_b_cpu, M_cpu, shadow_mask_cpu = batch[0], batch[1], batch[2], batch[3]
            # copy data to GPU
            ...
            loss_d_fake = torch.sum(criterionSoftplus(
                pred_fake)) / batchsize / w / h
            ...
            if iteration % 10 == 0:
                ...
                log['epoch'] = epoch
                log['iteration'] = len(
                    training_data_loader) * (epoch-1) + iteration
        ...
        with torch.no_grad():
            log_validation = test(
                config, validation_data_loader, gen, criterionMSE, epoch)
